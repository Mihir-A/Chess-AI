...
private:
    ...
    void draw();
    void drawGameResult();
    bool handleResultEvent(const SDL_Event &event);
    void drawRoundedRect(SDL_Rect rect, int radius, SDL_Color color) const;
    ...
    SDL_Point measureText(const std::string &text, float pixelHeight = 32.0f) const;
    void drawText(const std::string &text, int x, int y, const SDL_Color &color, float pixelHeight = 32.0f) const;
    void checkGameOver();
    void getMoves();
